...
class AmariloProject:
    ...
    async def get_item(self, session, url: str, project: str) -> dict:
    ...
    async def get_projects(self) -> list:
        async with ClientSession() as session:
            for project in list_endpoint:
                ...
                track_logs(url)
                self.all_projects_by_amarilo.append(asyncio.create_task(self.get_item(session, url, project)))
            response = await asyncio.gather(*self.all_projects_by_amarilo)
