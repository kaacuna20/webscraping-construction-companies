...
class ConalturaProject:
    ...
    async def get_item(self, session, url: str, index: int) -> dict:
        ...
        area = soup.find(name="h5", class_="text-conaltura-dark font-weight-bold descripciondetalle").text.split("m")[
            0].strip()
        # get a summary about the project
        description = soup.find(name="p", class_="text-conaltura-dark text-justify descripcion mb-5").text.replace("Ã¡",
                                                                                                                   "a").replace(
            "Ã³", "a").replace("Ãº", "u")
    ...
    async def get_projects(self) -> list:
        async with ClientSession() as session:
            ...
            for project in list_endpoint:
                ...
                url = f"{self.main_endpoint}{project}"
                track_logs(url)
                self.all_projects_by_catara.append(asyncio.create_task(self.get_item(session, url, index)))
                index += 1
                response = await asyncio.gather(*self.all_projects_by_catara)
        return response